...
tools for open, transparent, and reproducible analysis workflows. Working in a
reproducible framework represents a compromise between publishing a journal
article alone, and full-scale replication (i.e., the "gold standard"). Some 
[e.g., @peng11] have argued that reproducibility should be a basic,
*minimal*
standard as a means combating the so-called replicability crisis 
[see @hedges18]. We discuss R Markdown for integrating text with code and
*GitHub* for collaboration and documentation of the project history. Through 
the use of these tools, quantitative analyses are more open and transparent, 
and the likelihood of the analysis being reproducible is increased.
...
reproducible frameworks using R, R Markdown, and *git*/*GitHub*.

All participants should have at least a basic familiarity with R and be
comfortable with the idea of working in a scripting environment. Note that the
...
practice and apply the learned skills, all within the online platform. All
participants will have access to the full suite of DataCamp modules for one
month prior to, and one month following, our in-person training. Users with 
less experience with R will be asked to complete the 
...
conceptual understanding of open science and  can turn to the tools later.
This training is aimed at beginners who have little to no experience with R
Markdown or version control, but who feel comfortable learning code and have at least a basic understanding of R.
...
reproducible---and we may like to think that most research in education adheres
...
across 47 education and psychology journals, @stevens13
...
if you only have access to the published findings. It is important to note that
reproducibility does not imply "correctness", but rather transparency in
process. Indeed, part of the reason reproducible research is so important is
that it allows other researchers a means of verifying the process, analysis, 
and ultimately, the validity of study findings.
...
however, is a clear accounting on how to actually begin engaging in open and
reproducible research. This training seeks to fill that gap by providing an  
initial introduction to tools to help make the process more tractable. In
particular, we advocate for *conducting science publicly* through
...
being updated produced through code and updated automatically each time the
document is rendered. There is therefore no
...
Participants in this training will: (a) understand why reproducibility is an
increasingly important consideration for educational researchers; (b) be
introduced to tools, specifically R Markdown and *git*/*GitHub*, for carrying
out open and reproducible research; and (c) understand some of the remaining
...
### Hour 1: Introduction to open and reproducible research.
...
The format will be primarily lecture with slides, with brief breakout sessions 
...
Markdown, including delineating code chunks from standard text, creating
...
### Hour 3
In the third hour we will discuss moving R Markdown documents to different
formats, including basic PDF documents and Microsoft Word. We then discuss the 
...
the last 40 minutes we guide participants through creating a new repo and
pushing their existing project to that repo. We then walk them
through the process of making changes, committing those changes, and
pushing them to the repo. Finally, we walk participants through the basics of
...
*not* get pushed to the repo. By the end of Hour 4, all participants should
have created their first publicly viewable GitHub repo that documents the
