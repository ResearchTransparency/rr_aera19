...
to tools for open, transparent, and reproducible data analysis workflows.
Carrying out educational research in reproducible framework represents
...
reproducibility should be a basic, *minimal* standard as a means combating
the so-called replicability crisis [see @hedges18]. We highlight *R Markdown*,
a tool for integrating text with code, and *git*/*GitHub*, for documentation of the
project history and to support collaboration. Through the use of these tools,
quantitative analyses are more open and transparent, and the likelihood of the
analysis being reproducible and trustworthy can be increased.
...
specifically discuss moving from existing frameworks to reproducible frameworks
...
the presenters have partnered with the DataCamp organization (https://
www.datacamp.com) to help provide a platform for less experienced users to get
...
online platform. All participants will have access to the full suite of DataCamp
modules for the month prior to, and the month following, our in-person training.
...
routine procedures, such as how much missing data were involved and how the
missing data were handled, was extraordinarily difficult. Indeed, in the vast
majority of cases, this information was simply missing. These findings were
likely the result of, at least in part, researchers attempting to summarize
their study without becoming bogged down in the details. The findings, however,
...
accounting on how to actually begin engaging in open and reproducible research.
...
modified and updated as the project matures, along with *literate programming* a
...
massive gains in efficiency by all tables, figures, and in-text references to
statistics (e.g., sample means) being produced through code and updated
automatically each time the document is rendered. There is therefore no hand-
...
- understand why reproducibility is an
increasingly important consideration for educational researchers
- be introduced to tools, specifically R Markdown and *git*/*GitHub*, for carrying
out open and reproducible research
- understand some of the remaining
